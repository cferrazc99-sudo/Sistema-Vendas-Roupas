...
def modulo_gestao_caixa(uid_ativo):
    ...
    try:
        df_retiradas = pd.read_sql_query(
            "SELECT id, data, hora, 'Retirada' as tipo, descricao as desc, valor, tipo_evento FROM retirada_caixa WHERE id_usuario = ?", 
            conn, params=(uid_ativo,)
        )
        
        # Total apenas de pagamentos para revendedores
        tot_pagamentos_revenda = df_retiradas[df_retiradas['tipo_evento'] == "Pagamento Revendedor"]['valor'].sum()

        # Total apenas de retiradas de recursos gerais
        tot_retiradas_recurso = df_retiradas[df_retiradas['tipo_evento'] == "Retirada Recurso"]['valor'].sum()

                # IMPRESSÃO PARA VERIFICAÇÃO NO TERMINAL
        print("\n--- DADOS RETORNADOS DO BANCO DE DADOS ---")
        print(df_retiradas)
        print(f"Total Pagamentos Revenda: R$ {tot_pagamentos_revenda:,.2f}")
        print(f"Total Retiradas Recurso: R$ {tot_retiradas_recurso:,.2f}")
        print("-------------------------------------------\n")
    except Exception:
        df_retiradas = pd.DataFrame(columns=['id', 'data', 'hora', 'tipo', 'desc', 'valor', 'evento'])
    ...
    st.subheader("📊 Resumo Consolidado Histórico", help="Este painel exibe os indicadores financeiros acumulados desde o início das operações do sistema.")
    ...
    pgo_pendente_revenda = tot_pagamentos_revenda - tot_a_pagar_revenda # Pagamentos Pendentes considerando o que falta pagar de comissão para revenda
    pgt_pendente_vendas = tot_vendas_hist - tot_a_receber_cli # Pagamentos Pendentes considerando o que falta receber dos clientes
    # Criação da grid horizontal com 9 colunas (Mantido idêntico)
    mc1, mc2, mc3, mc4, mc5, mc6, mc7, mc8, mc9 = st.columns(9)
    
    # mc1.metric("Total a Receber (Clientes)", f"R$ {tot_a_receber_cli:,.2f}")
    mc1.metric(
        label="Total a Receber (Clientes)", 
        value=f"R$ {tot_a_receber_cli:,.2f}",
        help="Soma acumulada de todas as parcelas geradas para os clientes no sistema."
    )
    ...
    mc9.metric("Total Retirada Recursos", f"R$ {-tot_retiradas_recurso:,.2f}", delta=f"{-tot_retiradas_recurso:,.2f}")
